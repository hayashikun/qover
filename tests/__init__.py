...
class ServerTestCase(unittest.TestCase):
    ...
    def setUp(self):
        self.server = Server(max_workers=self.workers)
        self.channel = grpc.insecure_channel(f'localhost:{self.port}')

        start_event = threading.Event()

        def server_start(_self):
            _self.server.run(_self.port)

        def is_server_started(_self):
            return _self.server.server._state.stage == grpc._server._ServerStage.STARTED  # noqa
    ...
    def tearDown(self):
        self.server.server._state.termination_event.set()
